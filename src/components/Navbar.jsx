        ...
        <header className={"w-full py-5 sm:px-10 px-5 flex justify-between" +
            ...
            <nav className={"flex w-full screen-max-width"}>
                ...
                <div className={"flex flex-1 justify-center max-sm:hidden "}>
                    {navLists.map(nav => (
                        <div key={nav} className={"px-5 text-sm cursor-pointer " +
                            "text-gray hover:text-white transition-all duration-300"}>
                            {nav}
                        </div>
                    ))}
                </div>
                ...
                <div className={"flex items-baseline gap-7 max-sm:justify-end max-sm:flex-1"}>
                    <img src={searchImg} alt="search" width={18} height={18} className="img-nav-hover"/>
                    <img src={bagImg} alt={"bag"} width={18} height={18} className="img-nav-hover"/>
                </div>
